---
title: "P0 · Computer is a State Machine"
date: 2026-04-20
categories: [TEMU]
...
先写一段代码感受一下。下面这个 50 行的 C 程序是一台**完整**的玩具 CPU——只有三个寄存器、两条指令，但它跑得动：
...
2. **"纸带上的字符串" = "内存里的字节"**。图灵机"纸带无限长"是理想化；真实 CPU 的内存有限，但只要内存**够用**，它就是**图灵完备**的——能计算任何可计算函数
