...
> **一句话**：Sigmoid 是 Softmax 在 $N=2 $ 时的特例，两者本质上做同一件事。
...
PPT 第 9 页展示了关键实验。考虑这样一个情况：网络输出 $y_3 = -1000 $（对类别 3 极度自信地排除），正确答案是 class 1。
...
把网络的输出 $y_i' $ 理解为概率模型 $P(\text{class}=i | x) $，对训练集做最大似然估计：
